...
- **COGS per bag (proportionate lot cost)**: When loading a truck, COGS uses proportionate stock register cost per bag instead of `netWeight × pricePerKg`. Formula varies by purchase type:
  - **Farm Gate**: `costPerBag = (totalPayable + coldStoreCharges) / originalBags` — cold/warehouse charges paid separately by merchant
  - **Cold Store**: `costPerBag = totalPayable / originalBags` — all deductions still part of total cost
  - **Mandi**: `costPerBag = netPayable / originalBags` — includes cost of goods + all mandi charges (commission, aadhat, hammali, extra)
