...
- **Profit & Loss**: Auto-generated for selected FY. Revenue from cash inward entries by revenueType. Expenses include: Cost of Goods Sold (COGS) calculated from actual sale transactions (harvest `totalCostOfGoods` + `transportationCharges` + `otherCharges`; seed `totalCost` + `transportCharges` + `otherCharges`), operating expenses from cash outflows (excluding `capital_expense`, `aadhtiya`, and `supplier` which are inventory/balance sheet items), depreciation, and loan interest. Raw aadhtiya/supplier cash payments are excluded — only the cost of goods actually sold appears in P&L; unsold inventory stays as current assets on the Balance Sheet.
